...
def build_tallies(detector_cell, use_layers=False, use_delayed_low=False):
    ...
    if use_delayed_low:
        ...
        low_energies = np.linspace(1e3, 30e3, 1000) # FIXED: High resolution bins instead of 1 giant bin
...
def build_transformation_matrix(material_dictionary, save_prefix="Matrix_A", mode="macro"):
    ...
    for name, mat in iterator:
        ...
        mean_n, mean_g, _, _ = run_scan(mat, f"Library Generation: {name}") # Unpacks 4 values now
        if mode == 'macro':
...
def solve_mystery_target(mystery_target_name, Matrix_A=None, labels=None, save_prefix="Matrix_A_Macro", mode="macro", use_layers=False, use_delayed_low=False):
    ...
    if use_delayed_low and mean_low is not None:
        energies_low = low_energies[:-1] # Use the bin centers generated in build_tallies
...
def plot_pftna_spectra(mean_g, gamma_energies, mystery_target_name, mean_low=None):
    ...
    if mean_low is not None:
        ...
        ax_inset.set_title('Delayed 1–30 keV', fontsize=8)
        ax_inset.tick_params(labelsize=6)
        ax_inset.axvspan(5, 8, alpha=0.15, color='orange')
...
def main():
    parser = argparse.ArgumentParser(
        description="🧠 Forensic Scanner V1.5\nPulsed D-T + N-16 + 5D Layered + Delayed Low-Energy X-rays",
    ...
    if args.solve:
        print(f"\n[SYSTEM] Booting in {args.mode.upper()} mode...")
        solve_mystery_target(args.solve, save_prefix=prefix, mode=args.mode, use_layers=args.use_layers, use_delayed_low=args.use_delayed_low)
