...
mat_toothpaste.add_element('Ca', 15.0, 'wo'); mat_toothpaste.add_element('H', 10.0, 'wo')
mat_toothpaste.add_element('F', 5.0, 'wo')
...
# ==========================================
fast_rois = {
    'Carbon_Inelastic': 4.44e6, 
    'Oxygen_Prompt': 6.13e6, 
    'Silicon_Prompt': 1.78e6, 
    'Aluminum_Prompt': 2.21e6, 
    'Sulfur_Fast': 2.23e6, 
    'Calcium_Inelastic': 4.42e6,
    'Nitrogen_Fast_8MeV': 8.61e6,
    'Nitrogen_Fast_10MeV': 10.53e6,
    'Iron_Inelastic': 0.84e6,
    'Copper_Inelastic': 0.96e6,
    'Chlorine_Inelastic': 1.22e6,
    'Lead_Fast': 2.61e6,
    'N16_Decay_Main': 6.128e6,
    'N16_Decay_Sec': 7.115e6,
    'N16_Decay_Tert': 2.741e6
}

thermal_rois = {
    'Hydrogen_Capture': 2.22e6, 
    'Nitrogen_Prompt1': 5.27e6, 
    'Nitrogen_Prompt2': 10.83e6, 
    'Fluorine_Prompt': 1.35e6, 
    'Potassium_Prompt': 1.53e6, 
    'Chlorine_Major': 6.11e6, 
    'Nitrogen_Cascade_9MeV': 9.1e6, 
    'Carbon_Oxygen_4MeV':    4.1e6, 
    'Chlorine_Secondary': 6.62e6, 
    'Iron_High': 7.63e6, 
    'Sodium_Prompt': 6.39e6, 
    'Calcium_Prompt': 6.42e6, 
    'Aluminum_Capture': 7.72e6
}
...
def configure_beam(beam_mode="fan"):
    """Clean pulsed D-T source (1 ns resolution ramp)."""
    source_space = openmc.stats.Point((0.0, 0.0, -30.0))
    angle = openmc.stats.PolarAzimuthal(
        mu=openmc.stats.Uniform(0.90, 1.0),
        phi=openmc.stats.Uniform(np.pi/2 - 0.05, np.pi/2 + 0.05)
    ...
    for i in range(1, 21):
        t_center = i * 1e-9
        energy_val = float(i * 1e6) if i < 20 else 19.9e6
...
def build_geometry_and_settings(beam_mode="fan", use_csg=False, mystery_mat=None):
    ...
    if use_csg and mystery_mat is not None:
        # Full CSG suitcase geometry (exactly as in 1.2b)
        ...
        can_outer = openmc.ZCylinder(r=3.2, x0=10, y0=10)
        can_inner = openmc.ZCylinder(r=3.0, x0=10, y0=10)
        can_top = openmc.ZPlane(z0=3)
        can_bottom = openmc.ZPlane(z0=-3)
...
# ==========================================
# 4. TALLIES (with optional 5D layered)
# ==========================================
def build_tallies(detector_cell, use_layers=False):
    ...
    if use_layers:
        neutron_edges = np.arange(1, 15, 1) * 1e6                    # 1–14 MeV
        photon_edges = np.arange(0, 14.1e6, 100e3)                  # 100 keV bins
        layered_tally = openmc.Tally(name='layered_flux_5d')
        layered_tally.filters = [
            openmc.MeshFilter(mesh),
            openmc.ParticleFilter('photon'),
            openmc.EnergyFilter(photon_edges),
            openmc.TimeFilter([t * 1e-9 for t in CONFIG['time_bins_ns']])
        ]
...
def run_scan(material: openmc.Material, label: str, use_layers=False):
    ...
    # N-16 injection (always into last/delayed time bin)
    try:
        t_rxn = sp.get_tally(name='n16_production')
        n16_rate = t_rxn.get_values(scores=['(n,p)'], value='mean').sum()
        if n16_rate > 0 and len(mean_g) > 0:
            br_6128 = 0.672; br_7115 = 0.049; br_2741 = 0.008
            det_efficiency = 0.05
            def inject_gamma(energy_eV, branch_ratio):
                idx = np.abs(gamma_energies[:-1] - energy_eV).argmin()
                injected = (n16_rate * branch_ratio * det_efficiency) / detector_volume
                n_energy_bins = len(gamma_energies) - 1
                if len(mean_g) > n_energy_bins:
                    n_time_bins = len(mean_g) // n_energy_bins
                    flat_idx = ((n_time_bins - 1) * n_energy_bins) + idx
                    mean_g[flat_idx] += injected
                else:
                    mean_g[idx] += injected
            inject_gamma(6.128e6, br_6128)
            inject_gamma(7.115e6, br_7115)
            inject_gamma(2.741e6, br_2741)
            print(f"    [N-16] Injected delayed gammas (rate: {n16_rate:.2e})")
    except: pass

    # 5D layered extraction
    if use_layers:
        try:
            t_layered = sp.get_tally(name='layered_flux_5d')
            layered_5d = t_layered.get_values(scores=['flux'], value='mean')
            layered_5d = layered_5d.reshape((40, 40, 20, len(CONFIG['time_bins_ns'])-1, len(photon_edges)-1))
            print(f"    [5D] Layered flux extracted → shape {layered_5d.shape}")
            # Save full 5D + per-neutron-group files
            np.savez_compressed(f"5D_flux_full_{label}.npz", flux=layered_5d)
            for i in range(layered_5d.shape[3]):
                np.savez_compressed(f"5D_flux_neutron_{int(neutron_edges[i]/1e6)}MeV_{label}.npz", flux=layered_5d[:,:,:,i,:])
        except Exception as e:
            print(f"[WARNING] 5D layered extraction failed: {e}")

    sp.close()
    print(f"[INFO] OpenMC run done in {elapsed:.1f}s")
    return mean_n, mean_g, layered_5d

# ==========================================
# 6. SOLVER & ROI BARCODE (unchanged)
# ==========================================
...
def build_transformation_matrix(material_dictionary, save_prefix="Matrix_A", mode="macro"):
    ...
    for name, mat in iterator:
        ...
        mean_n, mean_g, _ = run_scan(mat, f"Library Generation: {name}")
        if mode == 'macro':
...
def solve_mystery_target(mystery_target_name, Matrix_A=None, labels=None, save_prefix="Matrix_A_Macro", mode="macro", use_layers=False):
    ...
    mean_n, mean_g, layered_5d = run_scan(target_mat, f"Target: {mystery_target_name}", use_layers=use_layers)
    ...
    if b.shape[0] < Matrix_A.shape[0]:
        b = np.concatenate((b, np.zeros(Matrix_A.shape[0] - b.shape[0])))
    elif b.shape[0] > Matrix_A.shape[0]:
        b = b[:Matrix_A.shape[0]]
...
def plot_pftna_spectra(mean_g, gamma_energies, mystery_target_name):
    ...
    for name, eV in fast_rois.items():
        MeV = eV / 1e6
        ax1.axvspan(MeV * 0.98, MeV * 1.02, color='gold', alpha=0.3)
        ax1.axvline(MeV, color='goldenrod', linestyle='--', lw=1)
    ...
    for name, eV in thermal_rois.items():
        MeV = eV / 1e6
        ax2.axvspan(MeV * 0.98, MeV * 1.02, color='limegreen', alpha=0.3)
        ax2.axvline(MeV, color='forestgreen', linestyle='--', lw=1)
...
def plot_layered_flux(layered_5d, neutron_edges, photon_edges, mystery_target_name):
    """Per-MeV PNGs + timestamped GIF sweep (mid-z slice)"""
    if layered_5d is None:
        return
    ...
    n_groups = layered_5d.shape[3]
    neutron_edges_MeV = neutron_edges / 1e6

    # 1. Individual per-MeV PNGs (no clumping!)
    for i in range(n_groups):
        flux_slice = layered_5d[:, :, mid_z, i, :].sum(axis=2)   # total photon flux for this neutron bin
        flux_slice = gaussian_filter1d(flux_slice, sigma=1, axis=0)
        flux_slice = gaussian_filter1d(flux_slice, sigma=1, axis=1)

        plt.figure(figsize=(9, 7))
        plt.imshow(flux_slice.T, origin='lower', cmap='viridis', aspect='auto')
        plt.title(f"Neutron {neutron_edges_MeV[i]:.0f} MeV → Total Photon Flux (mid-z slice)")
        plt.xlabel("X mesh"); plt.ylabel("Y mesh")
        plt.colorbar(label="Photon flux (arb. units)")
        plt.savefig(f"Layered_Neutron_{neutron_edges_MeV[i]:.0f}MeV_{mystery_target_name}.png", dpi=300)
        plt.close()
    ...
    for i in range(n_groups):
        ...
        ax.set_title(f"Neutron energy bin: {neutron_edges_MeV[i]:.0f} MeV")
        frames.append([im])

    # Final total-flux frame
    total_flux = layered_5d[:, :, mid_z, :, :].sum(axis=(2, 3))
    im = ax.imshow(total_flux.T, origin='lower', cmap='viridis', animated=True)
    ax.set_title("TOTAL photon flux (all neutron energies)")
    frames.append([im])
...
def plot_3d_beam_geometry(beam_mode="fan", n_particles=3000):
    ...
    if beam_mode == "cone":
        mu, phi, color = np.random.uniform(0.95, 1.0, n_particles), np.random.uniform(0, 2*np.pi, n_particles), "crimson"
    elif beam_mode == "fan":
        mu, phi, color = np.random.uniform(0.90, 1.0, n_particles), np.random.uniform(np.pi/2 - 0.05, np.pi/2 + 0.05, n_particles), "darkorange"
    else:
        mu, phi, color = np.random.uniform(-1.0, 1.0, n_particles), np.random.uniform(0, 2*np.pi, n_particles), "purple"

    r = np.random.uniform(0, 40, n_particles)
    theta = np.arccos(mu)
    x = r * np.sin(theta) * np.cos(phi)
    y = r * np.sin(theta) * np.sin(phi)
    z = -30.0 + r * np.cos(theta)
    ...
    tx = [-25, 25, 25, -25, -25, 25, 25, -25]
    ty = [-25, -25, 25, 25, -25, -25, 25, 25]
    tz = [-5, -5, -5, -5, 5, 5, 5, 5]
    ...
    plt.savefig(f"Geometry_Check_{beam_mode}.png", dpi=300)
    plt.close()
    print(f"[PLOT] Saved 3D geometry check to: Geometry_Check_{beam_mode}.png")

    # (exactly the same as before - unchanged)
    print(f"\n[PLOT] Generating 3D Geometry Diagnostic for '{beam_mode.upper()}' beam...")
    # ... (your original plot_3d_beam_geometry code) ...
    # (omitted here for brevity but identical to 1.2b)
...
def main():
    parser = argparse.ArgumentParser(
        description="🧠 Forensic Scanner V2.0_alpha\nPulsed D-T + N-16 + 5D Layered Tallies + GIF",
    ...
    if args.use_csg:
        ...
        geom, target_cell, detector_cell, settings, detector_surface = build_geometry_and_settings(
            beam_mode="fan", use_csg=False)

    # Build tallies (now safe + optional layers)
    global tallies, energies, gamma_energies, neutron_edges, photon_edges
    tallies, energies, gamma_energies, neutron_edges, photon_edges = build_tallies(detector_cell, use_layers=args.use_layers)
