...
def configure_beam(beam_mode="fan"):
    """Clean, production-ready pulsed D-T source (1 ns resolution ramp)."""
    # Point source completely bypasses spatial rejection checks
    ...
    # Ramp UP: 1 ns → 20 ns (1 MeV → 19.9 MeV)
    for i in range(1, 21):
        t_center = i * 1e-9
        ...
        s = openmc.IndependentSource(
            space=source_space,
            angle=angle,
            energy=openmc.stats.Discrete([energy_val], [1.0]),
            time=time_dist
        )
        s.particle = 'neutron'
        sources.append(s)

    # Ramp DOWN: 21 ns → 39 ns (19 MeV → 1 MeV)
...
def build_geometry_and_settings(beam_mode="fan", use_csg=False, mystery_mat=None):
    ...
    if use_csg and mystery_mat is not None:
        # (Full CSG suitcase geometry - unchanged from your 1.2)
        ...
        wire_left = openmc.XPlane(x0=-10)
        wire_right = openmc.XPlane(x0=10)
        ...
        paste_top = openmc.YPlane(y0=15)
        paste_bottom = openmc.YPlane(y0=5)
        ...
        # Simple geometry (for matrix building)
...
# 4. TALLIES
# ==========================================
def build_tallies(detector_cell):
    """Now takes detector_cell as argument so it can be called after geometry is built."""
    ...
    return openmc.Tallies([tally_n, tally_g, rxn_tally, tally_mesh]), energies, gamma_energies


# ==========================================
# 5. NORMALIZATION & SOLVER LOGIC
# ==========================================
...
def run_scan(material: openmc.Material, label: str):
    ...
    mean_n = mean_g = mean_ang = std_n = std_g = std_ang = np.array([])
    ...
    # === REAL N-16 ACTIVATION INJECTION (improved) ===
    try:
        ...
        if n16_rate > 0 and len(mean_g) > 0:
            ...
            def inject_gamma(energy_eV, branch_ratio):
                ...
                injected_flux = (n16_rate * branch_ratio * det_efficiency) / detector_volume
                
                n_energy_bins = len(gamma_energies) - 1
                if len(mean_g) > n_energy_bins:  # time-binned case
                    n_time_bins = len(mean_g) // n_energy_bins
                    flat_idx = ((n_time_bins - 1) * n_energy_bins) + idx  # ALWAYS last (delayed) bin
                    mean_g[flat_idx] += injected_flux
                else:
                    mean_g[idx] += injected_flux

            inject_gamma(6.128e6, br_6128)
            inject_gamma(7.115e6, br_7115)
            inject_gamma(2.741e6, br_2741)
            
            print(f"    [N-16] Injected delayed gammas (production rate: {n16_rate:.2e})")
    except Exception as e:
        print(f"[WARNING] N-16 activation injection failed: {e}")
    
    sp.close()
    print(f"[INFO] OpenMC run done in {elapsed:.1f}s, extracted data via Pandas.")
    return mean_n, std_n, mean_g, std_g, mean_ang, std_ang

# (The rest of your functions — safe_concat_vectors, extract_roi_barcode, 
# build_transformation_matrix, solve_mystery_target, plot_pftna_spectra, 
# plot_3d_beam_geometry, main() — are unchanged and kept exactly as in your 1.2)
...
def plot_pftna_spectra(mean_g, gamma_energies, mystery_target_name):
    ...
    ax1.set_title("Fast Inelastic Scattering Window", fontsize=14)
    ...
    ax2.set_title("Thermal Capture Window", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description="🧠 Forensic Scanner V1.2a (Clean Pulsed D-T + Real N-16 Activation)",
    ...
    # Now safe to build tallies
    global tallies, energies, gamma_energies
    tallies, energies, gamma_energies = build_tallies(detector_cell)
