#!/usr/bin/env python3
# ==============================================================================
# FORENSIC SCANNER V2.0.9 Updated 03/04/2026 - 13:44 CEST
# Pulsed D-T + N-16 + 5D Layered + Delayed Low-Energy X-rays + Bayesian NNFL
# Addons - Stochastic Noise + List-Mode Digitizer
# ==============================================================================
...
# ==========================================
# STOCHASTIC NOISE INJECTION EXTENDED
# ==========================================
def add_stochastic_noise(mean_array, std_array=None, noise_type='none', random_seed=None):
    """Add stochastic noise to a mean spectrum.
    noise_type:
        'none'     : return mean_array
        'poisson'  : Poisson(mean_array)
        'gaussian' : N(mean_array, std_array or sqrt(mean_array))
        'laplace'  : Laplace around mean_array
        'gamma'    : Gamma-distributed counts around mean_array
    """
    rng = np.random.default_rng(random_seed)
    mean_array = np.asarray(mean_array, dtype=float)

    if noise_type == 'none':
        return mean_array

    if noise_type == 'poisson':
        return rng.poisson(np.clip(mean_array, 0.0, None))

    if noise_type == 'gaussian':
        if std_array is None:
            std_array = np.sqrt(np.clip(mean_array, 0.0, None))
        std_array = np.asarray(std_array, dtype=float)
        return rng.normal(loc=mean_array, scale=std_array)

    if noise_type == 'laplace':
        if std_array is None:
            b = np.sqrt(np.clip(mean_array, 0.0, None) / 2.0)
        else:
            std_array = np.asarray(std_array, dtype=float)
            b = std_array / np.sqrt(2.0)
        noise = rng.laplace(loc=0.0, scale=b, size=mean_array.shape)
        out = mean_array + noise
        return np.clip(out, 0.0, None)

    if noise_type == 'gamma':
        alpha = 2.0
        out = np.zeros_like(mean_array, dtype=float)
        mask = mean_array > 0
        theta = np.zeros_like(mean_array, dtype=float)
        theta[mask] = mean_array[mask] / alpha
        out[mask] = rng.gamma(shape=alpha, scale=theta[mask])
        return out
...
# ==========================================
# FOKKER–PLANCK DIFFUSION (ENERGY-SPACE BROADENING)
# ==========================================
def fokker_planck_diffusion(
    spectrum,
    energy_bins,
    dt=1e-3,
    n_steps=10,
    ...
    # Apply 1D Fokker–Planck-like diffusion in energy space.
    spec = np.asarray(spectrum, dtype=float).copy()
    E = np.asarray(energy_bins, dtype=float)

    if spec.shape != E.shape:
        raise ValueError("fokker_planck_diffusion: spectrum and energy_bins must have same shape")

    dE = np.mean(np.diff(E))
    D = k * (a + b * E + c * E**2)
    D = np.clip(D, 0.0, None)

    total_counts = spec.sum()

    for _ in range(n_steps):
        p = spec.copy()
        lap = np.zeros_like(p)
        lap[1:-1] = (D[2:] * p[2:] - 2.0 * D[1:-1] * p[1:-1] + D[0:-2] * p[0:-2]) / (dE ** 2)
        lap[0] = (D[1] * p[1] - D[0] * p[0]) / (dE ** 2)
        lap[-1] = (D[-2] * p[-2] - D[-1] * p[-1]) / (dE ** 2)

        spec = p + dt * lap
        spec = np.clip(spec, 0.0, None)

        if conserve_norm and total_counts > 0:
            s = spec.sum()
            if s > 0:
                spec *= (total_counts / s)
...
# LIST-MODE DIGITIZER
# ==========================================
def build_listmode_tally(detector_cell):
    """True list-mode / collision tracking digitizer"""
    cell_filter = openmc.CellFilter([detector_cell])
    energy_bins = np.linspace(0.0, 12e6, 2400)

    tally_lm = openmc.Tally(name='listmode_digitizer')
    tally_lm.filters = [cell_filter, openmc.EnergyFilter(energy_bins)]
    tally_lm.scores = ['pulse-height']
    return tally_lm
...
CONFIG = {
    'sigma_certainty': 4,
    'particles_per_batch': 20_000_000, 
    'batches': 250,                
    'energy_min_eV': 1.0,
    'energy_max_eV': 1.4e7,
    'energy_bins': 2000,
    ...
    # New Fokker–Planck entries
    'use_fokker_planck': False,
    'fokker_planck_params': {
        'dt': 1e-3,
        'n_steps': 10,
        'k': 1.0,
        'a': 1.0,
    ...
    'no_n16_injection': False,   # new N-16 guard
 }
...
# 4. TALLIES - Updated
# ==========================================
def build_tallies(detector_cell, use_layers=False, use_delayed_low=False, target_cell=None):
    ...
    tally_n.filters = [cell_filter, neutron_filter, energy_filter]
    tally_n.scores = ['flux']
    ...
    # N-16 production tally - target-specific in micro mode
    rxn_tally = openmc.Tally(name='n16_production')
    rxn_tally.scores = ['(n,p)']
    rxn_tally.nuclides = ['O16']
    if target_cell is not None:
        rxn_tally.filters = [openmc.CellFilter([target_cell])]
        print("[N16 TALLY] Restricted to target cell (micro mode) → only material oxygen is activated")

    mesh = openmc.RegularMesh()
    mesh.dimension = [40, 40, 20]
    mesh.lower_left = [-25, -25, -5]
    mesh.upper_right = [25, 25, 5]
    ...
    tally_lm = build_listmode_tally(detector_cell)

    tallies_list = [tally_n, tally_g, rxn_tally, tally_mesh, tally_lm]
    ...
    if use_layers:
        ...
        layered_tally.scores = ['flux']
        tallies_list.append(layered_tally)
...
# ==========================================
# 7. NORMALIZATION & RUN SCAN - Updated
# ==========================================
...
def run_scan(material: openmc.Material, label: str, noise_type='none', noise_scale=1.0, 
             enable_listmode=False, use_layers=False, use_delayed_low=False):
    ...
    try: 
        ...
        raw_n = t_n.get_values(scores=['flux'], value='mean').flatten()
    except: 
        raw_n = np.array([])
    
    try: 
        t_g = sp.get_tally(name='pgaa_signal')
        raw_g = t_g.get_values(scores=['flux'], value='mean').flatten()
    except: 
        raw_g = np.array([])

        # === FOKKER–PLANCK DIFFUSION (applied to ideal photon spectrum) ===
        if CONFIG.get('use_fokker_planck', False):
            fp = CONFIG['fokker_planck_params']
            mean_g = fokker_planck_diffusion(
                mean_g,
                energy_bins=gamma_energies[:-1],
                dt=fp['dt'],
                n_steps=fp['n_steps'],
                k=fp['k'],
                a=fp['a'],
            ...
            print(f"[FP] Applied Fokker-Planck diffusion (preset: {CONFIG.get('fp_preset', 'custom')})")

    # === STOCHASTIC NOISE (apply AFTER FP) ===
    if noise_type != 'none':
        mean_n = add_stochastic_noise(raw_n, noise_type=noise_type)
        mean_g = add_stochastic_noise(raw_g, noise_type=noise_type)
        print(f"[NOISE] Applied {noise_type} noise (scale={noise_scale})")
    else:
        mean_n = raw_n
        mean_g = raw_g

    # === VOLUME NORMALIZATION ===
    if detector_volume > 1.0:
        mean_n = mean_n / detector_volume
        mean_g = mean_g / detector_volume

    # === LIST-MODE DIGITIZER ===
    if enable_listmode:
        try:
            t_lm = sp.get_tally(name='listmode_digitizer')
            pulse_data = t_lm.get_values(scores=['pulse-height'], value='mean').flatten()
            filename = f"listmode_events_{label.replace(' ', '_')}.npy"
            np.save(filename, pulse_data)
            print(f"[LIST-MODE] Saved {len(pulse_data):,} individual energy deposition events → {filename}")
        except Exception as e:
            print(f"[WARN] List-mode extraction failed: {e}")
...
# === N-16 INJECTION (now properly guarded) ===
    try:
        t_rxn = sp.get_tally(name='n16_production')
        n16_rate = t_rxn.get_values(scores=['(n,p)'], value='mean').sum()
        if (not CONFIG.get('no_n16_injection', False)) and n16_rate > 0 and len(mean_g) > 0:
            ...
            def inject_gamma(energy_eV, branch_ratio):
                ...
                if len(mean_g) > n_energy_bins:
                    ...
                    mean_g[idx] += injected
            inject_gamma(6.128e6, 0.672)
            inject_gamma(7.115e6, 0.049)
            inject_gamma(2.741e6, 0.008)
    ...
    # Kept consistent with the 4-variable unpack within `solve_mystery_target`
...
def solve_mystery_target(mystery_target_name, Matrix_A=None, labels=None, save_prefix="Matrix_A_Macro", mode="macro", noise_type='none', noise_scale=1.0, enable_listmode=False, use_layers=False, use_delayed_low=False, use_bayesian_nnfl=False):
    ...
    mean_n, mean_g, layered_5d, mean_low = run_scan(
        target_mat, f"Target: {mystery_target_name}", 
        noise_type=noise_type, 
        noise_scale=noise_scale,      # ← added
        enable_listmode=enable_listmode, 
        use_layers=use_layers, 
        use_delayed_low=use_delayed_low
    )

    if use_delayed_low and mean_low is not None:
        energies_low = low_energies[:-1]
        roi_counts, warnings_list = delayed_low_energy_analysis(mean_low, energies_low)
    ...
    # === BAYESIAN NNFL LAYER ===
    if use_bayesian_nnfl and Matrix_A.shape[1] == len(labels):
    ...
    if mode == 'micro':
        ...
        # 2. THE GHOST EXPLOSIVE TRIGGER (TATP/Peroxides)
        n_energy_bins = len(gamma_energies) - 1
        fast_spectrum = mean_g[:n_energy_bins]
        e_mids = gamma_energies[:-1]
        
        # Physically slice and sum the Carbon (4.44 MeV) and Oxygen (6.13 MeV) peaks
        c_counts = np.sum(fast_spectrum[(e_mids >= 4.44e6 * 0.98) & (e_mids <= 4.44e6 * 1.02)])
        o_counts = np.sum(fast_spectrum[(e_mids >= 6.13e6 * 0.98) & (e_mids <= 6.13e6 * 1.02)])
...
def main():
    parser = argparse.ArgumentParser(
        description="🧠 Forensic Scanner V2.0.4\nPulsed D-T + N-16 + 5D Layered + Delayed Low-Energy X-rays + Bayesian NNFL + Stochastic Noise + List-Mode Digitizer",
    ...
    parser.add_argument('--noise', type=str, choices=['poisson', 'gaussian', 'none'], default='none', help='Add stochastic noise to tallies')
    parser.add_argument('--noise-scale', type=float, default=1.0, help='Noise aggressiveness (default 1.0). Lower = less aggressive (e.g. 0.5)')
    ...
    parser.add_argument('--listmode', action='store_true', help='Enable full list-mode digitizer / collision tracking')
    
    # --- NEW: Fokker–Planck + N-16 flags ---
    parser.add_argument("--use-fokker-planck", action="store_true", help="Enable Fokker–Planck diffusion...")
    parser.add_argument("--fp-dt", type=float, default=1e-3, help=...)
    parser.add_argument("--fp-steps", type=int, default=10, help=...)
    parser.add_argument("--fp-k", type=float, default=1.0, help=...)
    parser.add_argument("--fp-a", type=float, default=1.0, help=...)
    parser.add_argument("--fp-b", type=float, default=0.0, help=...)
    parser.add_argument("--fp-c", type=float, default=0.0, help=...)
    parser.add_argument("--fp-no-conserve", action="store_true", help=...)
    parser.add_argument("--fp-preset", type=str, default=None, choices=["scintillator","semiconductor","harsh-training","metallic-energetics","chno-organics"], help=...)
    parser.add_argument("--no-n16-injection", action="store_true", help="Disable N-16 delayed-gamma injection...")

    args = parser.parse_args()

    CONFIG['use_fokker_planck'] = args.use_fokker_planck
    CONFIG['no_n16_injection'] = args.no_n16_injection

    fp_params = {
        "dt": args.fp_dt,
        "n_steps": args.fp_steps,
        "k": args.fp_k,
        "a": args.fp_a,
        "b": args.fp_b,
        "c": args.fp_c,
        "conserve_norm": not args.fp_no_conserve,
    }

    # Preset overrides (only one block — delete any duplicate below)
    if args.fp_preset == "scintillator":
        fp_params.update({"k": 0.8, "a": 1.0, "b": 0.015, "c": 0.0, "dt": 5e-4, "n_steps": 15})
    elif args.fp_preset == "semiconductor":
        fp_params.update({"k": 0.5, "a": 0.5, "b": 0.02, "c": 1e-4, "dt": 1e-3, "n_steps": 10})
    elif args.fp_preset == "harsh-training":
        fp_params.update({"k": 1.5, "a": 1.0, "b": 0.05, "c": 5e-4, "dt": 2e-3, "n_steps": 25})
    elif args.fp_preset == "metallic-energetics":
        fp_params.update({"k": 1.2, "a": 0.8, "b": 0.03, "c": 3e-4, "dt": 1e-3, "n_steps": 20})
    elif args.fp_preset == "chno-organics":
        fp_params.update({"k": 0.9, "a": 1.2, "b": 0.01, "c": 5e-5, "dt": 7e-4, "n_steps": 12})

    CONFIG['fokker_planck_params'] = fp_params
    CONFIG['fp_preset'] = args.fp_preset or "custom"   # for clean logging
    ...
    if args.use_csg:
        ...
        geom, target_cell, detector_cell, settings, detector_surface = build_geometry_and_settings(
            beam_mode="fan", use_csg=True, mystery_mat=target_mat)
    else:
        geom, target_cell, detector_cell, settings, detector_surface = build_geometry_and_settings(beam_mode="fan")

    # UPDATED tallies call with target_cell

    # Declare module-level globals so run_scan() and solve_mystery_target() can see them
    global tallies, energies, gamma_energies, neutron_edges, photon_edges, low_energies

    # UPDATED tallies call with target_cell (activates N-16 micro fix)
    tallies, energies, gamma_energies, neutron_edges, photon_edges, low_energies = build_tallies(
        detector_cell,
