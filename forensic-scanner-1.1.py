#!/usr/bin/env python3
# ==============================================================================
# 🧠 DUAL-MODE NNLS TOMOGRAPHY & SCANNER V1.1 (Pandas + Smart Thermal + Volume)
# ==============================================================================
...
fast_rois = {
    ...
    'Nitrogen_Fast_8MeV': 8.61e6,  # The giant missing peak!
    'Nitrogen_Fast_10MeV': 10.53e6, # The second giant missing peak!
    'Iron_Inelastic': 0.84e6,
    'Copper_Inelastic': 0.96e6,
    'Chlorine_Inelastic': 1.22e6,
    'Lead_Fast': 2.61e6,
    # The N-16 / O-16 Activation Signatures
...
def configure_beam(beam_mode="fan"):
    ...
    # Ramp UP: 1ns to 20ns (1 MeV to 20 MeV)
    for i in range(1, 21):
        t_center = i * 1e-9
        energy_val = i * 1e6
        # Create a 1ns wide pulse window
    ...
    # Ramp DOWN: 21ns to 39ns (19 MeV down to 1 MeV)
    ...
    # Normalize source strengths so they sum to 1.0
    for s in sources:
        s.strength = 1.0 / len(sources)
"""
""" Third old beam tyoe 
def configure_beam(beam_mode="fan"):
    # FIX: Give the box a tiny 3D thickness so OpenMC doesn't segfault on a 2D plane!
    source_space = openmc.stats.Box([-0.5, -0.5, -30.01], [0.5, 0.5, -29.99])
    ...
    # Normalize source strengths so they sum perfectly to 1.0
    for s in sources:
        s.strength = 1.0 / len(sources)
    return sources
"""
""" Fourth KAPUTT BEAM 
def configure_beam(beam_mode="fan"):
    # FOOLPROOF FIX: A Point source completely bypasses spatial rejection checks!
    # The beam originates exactly here and diverges outward based on the angle.
    ...
    for i in range(1, 21):
        t_center = i * 1e-9
        energy_val = float(i * 1e6) # Ensure pure float for the Discrete sampler
        time_dist = openmc.stats.Uniform(t_center - 0.5e-9, t_center + 0.5e-9)
        s = openmc.IndependentSource(space=source_space, angle=angle, energy=openmc.stats.Discrete([energy_val], [1.0]), time=time_dist)
        s.particle = 'neutron'
        sources.append(s)
...
def configure_beam(beam_mode="fan"):
    # We use a Point source to completely bypass spatial bounds rejection
    source_space = openmc.stats.Point((0.0, 0.0, -30.0))
    ...
    for i in range(1, 21):
        t_center = i * 1e-9
        # THE FIX: Cap the peak at 19.9 MeV to prevent ENDF table out-of-bounds crashes!
...
def build_geometry_and_settings(beam_mode="cone", use_csg=False, mystery_mat=None):
...
def build_tallies():
    ...
    # --- ARTIFICIAL DELAYED GAMMA TALLY (N-16 Activation) ---
    # We tally the exact (n,p) reaction rate on Oxygen-16 everywhere in the geometry
    rxn_tally = openmc.Tally(name='n16_production')
    rxn_tally.scores = ['(n,p)'] # MT=103
    ...
    return openmc.Tallies([tally_n, tally_g, tally_ang, rxn_tally, tally_mesh]), energies, gamma_energies, polar_bins
...
# --- ROBUST TALLY EXTRACTION & NORMALIZATION ---
    ...
    # --- DELAYED GAMMA INJECTION (The Physics Cheat Code) ---
    try:
        t_rxn = sp.get_tally(name='n16_production')
        # Total O-16(n,p)N-16 reactions per source particle
        n16_rate = t_rxn.get_values(scores=['(n,p)'], value='mean').sum()
        
        if n16_rate > 0 and len(mean_g) > 0:
            # Empirical Branching Ratios for N-16 Beta Decay
            br_6128 = 0.672  # 67.2%
            br_7115 = 0.049  # 4.9%
            br_2741 = 0.008  # 0.8%
            
            # Since we bypass the Monte Carlo transport for these delayed gammas,
            # we apply a flat geometric efficiency factor (tune this if the peaks are too tall/short)
            det_efficiency = 0.05 
            
            def inject_gamma(energy_eV, branch_ratio):
                # Find the exact bin in our 2000-bin array that matches this energy
                idx = np.abs(gamma_energies[:-1] - energy_eV).argmin()
                # Add the flux directly to the spectrum
                injected_flux = (n16_rate * branch_ratio * det_efficiency) / detector_volume
                
                # If we are using time bins, inject it into the LAST time bin (delayed window)
                if len(mean_g) > len(gamma_energies) - 1:
                    n_energy_bins = len(gamma_energies) - 1
                    n_time_bins = len(mean_g) // n_energy_bins
                    # Inject into the deepest thermal/delayed time bin
...
# --- ISOTOPIC RATIO CHECKER (N-16 Activation) ---
    print("\n 🔬 ISOTOPIC RATIO DIAGNOSTICS:")
    e_mids = gamma_energies[:-1]
    
    # Helper to find flux near a specific energy
    def get_peak_flux(target_energy):
        idx = np.abs(e_mids - target_energy).argmin()
        return mean_g[idx] if len(mean_g.shape) == 1 else mean_g.reshape((-1, len(e_mids)))[0, idx]

    f_6128 = get_peak_flux(6.128e6)
    f_7115 = get_peak_flux(7.115e6)
    f_2741 = get_peak_flux(2.741e6)

    # Normalize the ratio based on the 1% tertiary peak (if it exists above noise)
    if f_2741 > 1e-8: 
        r_main = f_6128 / f_2741
        r_sec = f_7115 / f_2741
        print(f"    ➤ Detected N-16 Gamma Lines: 6.13 MeV ({f_6128:.2e}) | 7.12 MeV ({f_7115:.2e}) | 2.74 MeV ({f_2741:.2e})")
        print(f"    ➤ Empirical Ratio: {r_main:.1f} : {r_sec:.1f} : 1.0 (Expected ~67 : 5 : 1)")
        
        # If the main peak is massively dominant and the secondary is present, flag it
        if r_main > 30.0 and r_sec >= 1.0:
            threat_detected = True
            print("    ⚠️  STRONG O-16 ACTIVATION DETECTED: Coincident N-16 decay ratios confirmed!")
    else:
        print("    ➤ N-16 Gamma lines not detected above noise floor.")
