# Forensic Scanner V2.0.9 Final    : Monte Carlo PFTNA Engine
...
###### This system uses OpenMC to fire 14 MeV neutrons into complex 3D CSG (like shielded luggage), tracks subatomic particle collisions, and uses a Non-Negative Least Squares (NNLS) solver to mathematically unmix and identify hidden explosive threats from the resulting gamma-ray spectra.
...
### Step 2.1: Build the Transformation Matrix
    ...
    python forensic-scanner-2.0.9-Final.py --build-matrix --mode micro
    python forensic-scanner-2.0.9-Final.py --build-matrix --mode macro

### Step 2.2: Run a detection scan

    python forensic-scanner-2.0.9-Final.py --solve Threat_NQ --use-csg --mode micro
    python forensic-scanner-2.0.9-Final.py --solve Threat_NQ --use-csg --mode macro

### Step 2.3: Do some more brain

    python forensic-scanner-2.0.9-Final.py --solve Threat_NQ --use-csg --mode macro --use-layers --use-delayed-low --use-bayesian-nnfl
    python forensic-scanner-2.0.9-Final.py --solve TATP_Ghost --use-csg --mode macro --use-layers --use-delayed-low --use-bayesian-nnfl

### Step 2.3.5 Lets go ultra brain

    
    python forensic-scanner-2.0.9-Final.py --solve=Threat_NQ --use-fokker-planck --fp-preset=scintillator --use-delayed-low --listmode
    python forensic-scanner-2.0.9-Final.py --solve=Metal_Steel --use-fokker-planck --fp-preset=metallic-energetics --noise=laplace
    python forensic-scanner-2.0.9-Final.py --solve=TATP_Ghost --use-fokker-planck --fp-preset=chno-organics --use-bayesian-nnfl --noise=gamma

    python forensic-scanner-2.0.9-Final.py --build-matrix --mode=macro --use-fokker-planck --fp-preset=harsh-training --no-n16-injection
    python forensic-scanner-2.0.9-Final.py --solve=Threat_NQ --use-fokker-planck --fp-preset=chno-organics --use-delayed-low --listmode

###### You van figure out how to do all of the funky stuff with --help or look at the argparses

### Step 2.4: Options to check beam output visually

    python forensic-scanner-2.0.9-Final.py --check-geometry fan
    python forensic-scanner-2.0.9-Final.py --check-geometry cone

### Step 2.5: Now with more options

#### With realistic Poisson noise
    python forensic-scanner-2.0.9-Final.py --solve Threat_NQ --use-csg --mode macro --noise poisson

#### With full list-mode tracking (saves individual events)
    python forensic-scanner-2.0.9-Final.py --solve Threat_NQ --use-csg --mode macro --listmode

#### Both noise + list-mode
    python forensic-scanner-2.0.9-Final.py --solve Threat_NQ --use-csg --mode macro --noise poisson --listmode
